 ...
 * 
 */
//REQUIRED FOR GCC TO INCLUDE ASPRINTF
#define _GNU_SOURCE
...
obj_String String_method_PLUS(obj_String this, obj_String other){
  char* text1 = this->text;
  char* text2 = other->text;
  int len = strlen(text1) + strlen(text2) + 1;
  char* new_text = malloc(sizeof(char) * len);
  memset(new_text, '\0', len);
  strcat(new_text, text1);
  strcat(new_text, text2);
  return str_literal(new_text);
